  ...
  "mcpServers": {
    "task-master-ai": {
      ...
      "env": {
        ...
        "VERTEX_PROJECT_ID": "VERTEX_PROJECT_ID_HERE",
        "VERTEX_LOCATION": "VERTEX_LOCATION_HERE",
        "GOOGLE_APPLICATION_CREDENTIALS": "GOOGLE_APPLICATION_CREDENTIALS_HERE",
...
- `TAVILY_API_KEY` (Tavily search and research)

**For VertexAI models specifically:**
- `GOOGLE_API_KEY` OR `GOOGLE_APPLICATION_CREDENTIALS` (authentication)
...
**Common Issues:**
- **"Project ID not found"**: Ensure `VERTEX_PROJECT_ID` is set to your actual Google Cloud project ID
- **"Authentication failed"**: Verify either `GOOGLE_API_KEY` or `GOOGLE_APPLICATION_CREDENTIALS` is properly configured
- **"Region not available"**: Check that `VERTEX_LOCATION` is set to a valid Google Cloud region (default: us-central1)
- **"Model not found"**: Use latest model IDs like `gemini-2.5-flash` or `gemini-2.5-pro` (June 2025)
