...
function isApiKeySet(providerName, session = null, projectRoot = null) {
	...
	if (providerName?.toLowerCase() === 'ollama' || providerName?.toLowerCase() === 'context7') {
	...
	const keyMap = {
		...
		context7: null, // Context7 uses MCP tools, no API key needed
	...
	// Special handling for Vertex AI - can use service account credentials instead of API key
	if (providerKey === 'vertex') {
		const apiKeyValue = resolveEnvVariable('GOOGLE_API_KEY', session, projectRoot);
		const credentialsPath = resolveEnvVariable('GOOGLE_APPLICATION_CREDENTIALS', session, projectRoot);
		const projectId = resolveEnvVariable('VERTEX_PROJECT_ID', session, projectRoot);

		// Check if API key is valid
		const hasValidApiKey = apiKeyValue &&
			apiKeyValue.trim() !== '' &&
			!/YOUR_.*_API_KEY_HERE/.test(apiKeyValue) &&
			!apiKeyValue.includes('KEY_HERE');

		// Check if service account credentials are valid
		const hasValidCredentials = credentialsPath &&
			credentialsPath.trim() !== '' &&
			!credentialsPath.includes('CREDENTIALS_HERE');

		// Check if project ID is valid
		const hasValidProjectId = projectId &&
			projectId.trim() !== '' &&
			!projectId.includes('PROJECT_ID_HERE');

		// Return true if we have (API key OR credentials) AND project ID
		return (hasValidApiKey || hasValidCredentials) && hasValidProjectId;
	}
...
function getMcpApiKeyStatus(providerName, projectRoot = null) {
	...
	try {
		...
		switch (providerName) {
			...
			case 'context7':
				return true; // No key needed - uses MCP tools
			case 'tavily':
				apiKeyToCheck = mcpEnv.TAVILY_API_KEY;
				placeholderValue = 'YOUR_TAVILY_API_KEY_HERE';
				break;
			case 'mistral':
			...
			case 'azure':
				...
				break;
			case 'vertex':
				// VertexAI requires either API key OR service account credentials AND project ID
				const hasApiKey = mcpEnv.GOOGLE_API_KEY &&
					mcpEnv.GOOGLE_API_KEY !== 'YOUR_GOOGLE_API_KEY_HERE' &&
					mcpEnv.GOOGLE_API_KEY !== 'GOOGLE_API_KEY_HERE';

				const hasCredentials = mcpEnv.GOOGLE_APPLICATION_CREDENTIALS &&
					mcpEnv.GOOGLE_APPLICATION_CREDENTIALS !== 'GOOGLE_APPLICATION_CREDENTIALS_HERE';

				const hasProjectId = mcpEnv.VERTEX_PROJECT_ID &&
					mcpEnv.VERTEX_PROJECT_ID !== 'VERTEX_PROJECT_ID_HERE';

				// Return true if we have (API key OR credentials) AND project ID
				return (hasApiKey || hasCredentials) && hasProjectId;
			default:
