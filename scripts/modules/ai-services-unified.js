...
function _resolveApiKey(providerName, session, projectRoot = null) {
	...
	if (providerName === 'ollama' || providerName === 'bedrock' || providerName === 'context7') {
		return apiKey || null;
	}
	...
	if (providerName === 'vertex') {
		...
		if (credentialsPath) {
			// Service account credentials available, API key not required
			return apiKey || null;
		}
	}
	...
	for (const currentRole of sequence) {
		...
		try {
			...
			if (providerName?.toLowerCase() === 'vertex') {
				...
				// Load service account credentials if path is provided
				...
				if (credentialsPath) {
					try {
						...
						log('debug', `Loaded Google service account credentials from ${credentialsPath}`);
					} catch (error) {
						log('warn', `Failed to load Google service account credentials from ${credentialsPath}: ${error.message}`);
					}
				}
