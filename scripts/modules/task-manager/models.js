...
async function getModelConfiguration(options = {}) {
	...
	if (!configExists) {
		throw new Error(
			'The configuration file is missing. Run "guidant models --setup" to create it.'
		);
	}
		...
		if (providerHint) {
			...
			if (modelData && modelData.provider === providerHint) {
				...
				if (providerHint === 'openrouter') {
					...
					// Set provider without model validation since VertexAI models are managed by Google Cloud
					determinedProvider = 'vertex';
					warningMessage = `Warning: Custom VertexAI model '${modelId}' set. Please ensure the model ID is valid and accessible in your Google Cloud project.`;
					report('warn', warningMessage);
				} else {
