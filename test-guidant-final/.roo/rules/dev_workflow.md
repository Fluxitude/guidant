...
-   Mark completed tasks with `set_task_status` / `guidant set-status --id=<id> --status=done` (see [`guidant.md`](mdc:.roo/rules/guidant.md))
-   Update dependent tasks when implementation differs from original plan using `update` / `guidant update --from=<id> --prompt="..."` or `update_task` / `guidant update-task --id=<id> --prompt="..."` (see [`guidant.md`](mdc:.roo/rules/guidant.md))
...
2.  **`guidant` CLI (For Users & Fallback)**:
    ...
    - Install globally with `npm install -g guidant` or use locally via `npx guidant ...`.
    - The CLI commands often mirror the MCP tools (e.g., `guidant list` corresponds to `get_tasks`).
    - Refer to [`guidant.md`](mdc:.roo/rules/guidant.md) for a detailed command reference.
