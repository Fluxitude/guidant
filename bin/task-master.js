...
function createDevScriptAction(commandName) {
	return (options, cmd) => {
		...
		if (camelCaseFlags.length > 0) {
			...
			console.error(
				'\nExample: guidant parse-prd --num-tasks=5 instead of --numTasks=5\n'
			);
			process.exit(1);
		}
...
program
	.name('guidant')
	.description('Guidant AI Task Management CLI')
	.version(version)
...
process.on('uncaughtException', (err) => {
	...
	if (err.code === 'commander.unknownOption') {
		...
		console.error(
			chalk.yellow(
				`Run 'guidant ${command} --help' to see available options for this command`
			)
		);
		process.exit(1);
	}
	...
	if (err.code === 'commander.unknownCommand') {
		...
		console.error(
			chalk.yellow(`Run 'guidant --help' to see available commands`)
		);
		process.exit(1);
	}
...
// Show help if no command was provided (just 'guidant' with no args)
